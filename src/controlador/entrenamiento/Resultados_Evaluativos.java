...
public class Resultados_Evaluativos extends GenericForwardComposer {
	Window winResEvaluativos;
	Combobox cmbEficiencia, cmbEscala1, cmbEscala2, cmbEscala3, cmbCategoria,
			cmbEquipo;
...
//		lista.setModel(new ModeloListBox(matriz));
	...
	public void onSelect$cmbEficiencia() {
		ahEficiencia.setLabel(cmbEficiencia.getSelectedItem().getLabel());
	}


	public void onSelect$cmbEscala1() {
		int index = 2;
		for (int i = 0; i < lista.getItemCount(); i++) {
			...
			combo.setValue(cmbEscala1.getSelectedItem().getLabel());
		}
	}

	public void onSelect$cmbEscala2() {
		int index = 3;
		for (int i = 0; i < lista.getItemCount(); i++) {
			Listcell cell = (Listcell) lista.getItemAtIndex(i).getChildren()
					.get(3);
			Combobox combo = (Combobox) cell.getChildren().get(0);
			combo.setValue(cmbEscala2.getSelectedItem().getLabel());
		}

	}

	public void onSelect$cmbEscala3() {
		int index = 4;
		for (int i = 0; i < lista.getItemCount(); i++) {
			Listcell cell = (Listcell) lista.getItemAtIndex(i).getChildren()
					.get(4);
