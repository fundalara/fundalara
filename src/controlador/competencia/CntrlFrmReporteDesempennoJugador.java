...
public class CntrlFrmReporteDesempennoJugador extends GenericForwardComposer {
	...
	public void restaurar(){
		...
		System.out.println(servicioCompetencia.listarAperturadasClausurada().size());
	...
	public void onSelect$cmbCategoria() {
		...
		equipos = servicioEquipoCompetencia.listarEquipoPorCompetenciaCategoria(competencia,categoriaCompetencia);
		modalidades = servicioDatoBasico.listarPorTipoDato("MODALIDAD INDICADOR");
		
		binder.loadAll();
	}
	
	public void onSelect$cmbEquipo(){
		equipoCompetencia = (EquipoCompetencia) cmbEquipo.getSelectedItem().getValue();
	...
	public void onClick$btnGenerar()throws JRException, IOException, InterruptedException, SQLException{
		if (cmbCompetencia.getText().equalsIgnoreCase("----Seleccione----")) { 
		...
		}else if (cmbEquipo.getText().equalsIgnoreCase("----Seleccione----")) { 
			throw new WrongValueException(cmbCategoria, "Debe seleccionar una categoria");
		}else if (cmbModalidad.getText().equalsIgnoreCase("----Seleccione----")) { 
			throw new WrongValueException(cmbCategoria, "Debe seleccionar una categoria");
			}
			con = ConeccionBD.getCon("postgres", "postgres", "123456");
			parameters.put("codEquipo", equipoCompetencia.getEquipo().getCodigoEquipo());
			...
			String rutaReporte = Sessions.getCurrent().getWebApp().getRealPath("/WEB-INF/reportes/desempenoporequipo.jrxml");
			...
			final AMedia amedia = new AMedia("desempenoporequipo.pdf", "pdf", "application/pdf", archivo);
