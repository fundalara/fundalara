...
public class CntrlCuentaPagarPorProveedor extends GenericForwardComposer {
	Material mat;
	...
	public void onClick$btnFiltrar() throws WrongValueException, ParseException{
		...
		if (dtInicio.getText().equals("") || dtFin.getText().equals("")){
			facturas = servicioCuentaPagar.listarCuentaPorPagarFiltroPersona(paramEstado,persona);
			calcularMonto();
			
		}else {
			facturas = servicioCuentaPagar.listarCuentaPorPagarPorFechaPersona(dtInicio.getText().toString(), dtFin.getText().toString(),paramEstado,persona);
			calcularMonto();
		}
	...
	public void onSelect$lbxFacturas() {
		Listcell celda = (Listcell) lbxFacturas.getSelectedItem()
				.getChildren().get(0);
		...
		if (ch.isChecked()) {
			...
			Listcell celda7 = (Listcell) lbxFacturas.getSelectedItem()
					.getChildren().get(6);
			Doublebox mont = (Doublebox) celda7.getChildren().get(0);
			mont.setValue(facturas.get(lbxFacturas.getSelectedIndex()).getSaldo());
			materiales2 = servicioCuentaPagarMaterial
					.buscarMaterialesPorCuentaPagar(facturas.get(lbxFacturas.getSelectedIndex()));
		}
